...
async function carregarFuncionarios() {
  ...
  try {
    ...
    if (FECHAMENTO_RULES.podeSelecionarFuncionario(usuario)) {
      ...
      if (!lista.length) {
        showStatus('status-busca', 'Nenhum funcionário ativo encontrado para esta loteria.', 'err');
      } else {
    ...
    showStatus('status-busca', 'Erro ao carregar funcionários: ' + e.message, 'err');
  }
}
...
async function buscarFechamentoExistente() {
  ...
  try {
    ...
    showStatus('status-busca', '✓ Fechamento carregado para edição.', 'ok');
  } catch (e) {
...
function renderQuebra(quebra, cred, deb) {
  ...
  det.textContent = `Créditos (R$ ${fmtBRLs(cred)}) − Débitos (R$ ${fmtBRLs(deb)})`;
