...
function setClock(){ $('relogio').textContent = new Date().toLocaleTimeString('pt-BR') + ' — ' + new Date().toLocaleDateString('pt-BR'); }
setClock(); setInterval(setClock,1000);

function showStatus(id,msg,t='ok'){ const el=$(id); el.textContent=msg; el.className=`status-bar show ${t}`; }
function hideStatus(id){ const el=$(id); if(el) el.className='status-bar'; }
...
function renderFechamentoResumo(){
  $('tbody-fechamento-resumo').innerHTML = state.vendasFuncionario.length ? state.vendasFuncionario.map(v=>`<tr><td>${v.funcionario_nome}</td><td class="mono">${v.concurso}</td><td class="mono">${v.qtd_vendida}</td><td class="money">${fmtMoney(v.total_vendido)}</td></tr>`).join('') : `<tr><td colspan="4"><div class="empty"><div class="empty-title">Sem vendas lançadas</div></div></td></tr>`;
}
  ...
  try{
    ...
    if(state.editingCadastroConcurso){
      ...
      for(const item of mapa){
        ...
        const { error } = await sb.from('federais').insert(payload);
        if(error) throw error;
      }
...
window.deleteCadastro = async function(concurso){
  if(!concurso) return;
  const ok = confirm(`Apagar o concurso ${concurso} em todas as loterias?`);
  if(!ok) return;
  ...
  try{
    ...
    if(idsFederais.length){
      const { error: ePremio } = await sb
        .from('federal_encalhe_premio')
        .delete()
        .in('federal_id', idsFederais);
      if(ePremio) throw ePremio;

      const { error: eMov } = await sb
        .from('federal_movimentacoes')
        .delete()
        .in('federal_id', idsFederais);
      if(eMov) throw eMov;

      const { error: eItensFech } = await sb
        .from('fechamento_federais')
        .delete()
        .in('federal_id', idsFederais);
...
window.editMov = function(id){
  ...
  $('mov-total').value=(m.valor_total_real || m.valor_total || (Number(m.qtd_fracoes||0)*Number(m.valor_fracao_real||m.valor_fracao||0))).toFixed ? Number(m.valor_total_real || m.valor_total || (Number(m.qtd_fracoes||0)*Number(m.valor_fracao_real||m.valor_fracao||0))).toFixed(2) : '';
...
window.deleteMovDirect = async function(id){
  ...
  try{
    ...
    if(String(state.editingMovId || '') === String(id)){
      clearMov();
    }
...
async function saveFechamento(){
  try{
    ...
    if(!$('fec-loteria').value || !$('fec-usuario').value || !$('fec-data-ref').value || !$('fec-federal').value){ showStatus('st-fechamento','Preencha loja, funcionário, data e concurso.','err'); return; }
    ...
    const header={
      ...
      troco_inicial:0,troco_sobra:0,relatorio:0,deposito:0,pix_cnpj:0,diferenca_pix:0,premio_raspadinha:0,resgate_telesena:0,total_produtos:0,total_federais:totalFed,total_boloes:0,total_fiado:0,total_debitos:0,total_creditos:0,quebra:0,
      criado_por:state.usuario?.id || null,
      updated_at:new Date().toISOString()
    };
    ...
    const { error: eItem } = await sb.from('fechamento_federais').insert(item);
    if(eItem) throw eItem;
    showStatus('st-fechamento','Venda do funcionário registrada.','ok');
    $('fec-qtd-vendida').value=''; $('fec-total').value='';
    await refreshAll();
...
window.openFederalDetail = function(federalId){
  ...
  openDrawer(`Federal ${resumo?.concurso || ''}`, `${resumo?.loja_origem || ''} • ${fmtDate(resumo?.dt_sorteio)}`, `
    ...
    <div class="table-wrap"><table class="table"><thead><tr><th>Funcionário</th><th>Qtd</th><th>Total</th></tr></thead><tbody>${vendas.length ? vendas.map(v=>`<tr><td>${v.funcionario_nome}</td><td class="mono">${v.qtd_vendida}</td><td class="money">${fmtMoney(v.total_vendido)}</td></tr>`).join('') : '<tr><td colspan="3" class="muted">Sem vendas lançadas</td></tr>'}</tbody></table></div>
    <div class="sep"><span class="sep-label">Eventos</span><div class="sep-line"></div></div>
    <div class="table-wrap"><table class="table"><thead><tr><th>Evento</th><th>Origem</th><th>Destino</th><th>Qtd</th><th>Real</th></tr></thead><tbody>${movs.length ? movs.map(m=>`<tr><td>${m.tipo_evento||m.tipo}</td><td>${lookupLoteriaName(m.loteria_origem)}</td><td>${m.loteria_destino ? lookupLoteriaName(m.loteria_destino) : '—'}</td><td class="mono">${m.qtd_fracoes}</td><td class="money">${fmtMoney(m.valor_total_real || m.valor_total || (Number(m.qtd_fracoes||0)*Number(m.valor_fracao_real||m.valor_fracao||0)))}</td></tr>`).join('') : '<tr><td colspan="5" class="muted">Sem eventos</td></tr>'}</tbody></table></div>
  `, [{label:'Fechar', kind:'secondary', onClick:closeDrawer}]);
};

window.openMovDetail = function(id){
  const m=state.movimentos.find(x=>String(x.id)===String(id)); if(!m) return;
  openDrawer(`Movimentação ${m.tipo_evento || m.tipo}`, `${m.federais?.modalidade || 'Federal'} • Concurso ${m.federais?.concurso || '—'}`, `
    <div class="grid-2">
  ...
  `,[{label:'Editar',kind:'amber',onClick:()=>{closeDrawer(); editMov(id);}},{label:'Fechar',kind:'secondary',onClick:closeDrawer}]);
};

function openDrawer(title, sub, bodyHtml, actions=[]){
  $('drawer-title').textContent=title; $('drawer-sub').textContent=sub; $('drawer-body').innerHTML=bodyHtml; $('drawer-foot').innerHTML='';
  actions.forEach(a=>{ const b=document.createElement('button'); b.textContent=a.label; b.className = a.kind==='primary'?'btn-primary':a.kind==='amber'?'btn-amber':a.kind==='danger'?'btn-danger':'btn-secondary'; b.onclick=a.onClick; $('drawer-foot').appendChild(b); });
...
function bindEvents(){
  $('overlay').addEventListener('click', closeDrawer); $('btn-close-drawer').addEventListener('click', closeDrawer);
  $('overlay-lanc').addEventListener('click', closeLancamento); $('btn-close-lanc').addEventListener('click', closeLancamento); $('btn-cancel-lanc').addEventListener('click', closeLancamento); $('btn-save-lanc').addEventListener('click', saveLancamento);
  $('btn-filtrar-visao').addEventListener('click', renderVisao);
  $('btn-limpar-visao').addEventListener('click', ()=>{ ['filtro-concurso','filtro-loja','filtro-dt-ini','filtro-dt-fim'].forEach(id=>$(id).value=''); renderVisao(); });
  ...
  $('cad-data-prev').addEventListener('click', ()=>{ $('cad-dt-sorteio').value = nextQuaSabFrom($('cad-dt-sorteio').value || suggestNextSorteio(), -1); fillQtdPadraoCadastro(); });
  $('cad-data-next').addEventListener('click', ()=>{ $('cad-dt-sorteio').value = nextQuaSabFrom($('cad-dt-sorteio').value || suggestNextSorteio(), 1); fillQtdPadraoCadastro(); });
  ...
  $('mov-federal').addEventListener('change', ()=>{
    ...
    syncMovValorByTipo();
    $('mov-resumo-selec').innerHTML=`<div class="inline-pills"><span class="pill">Modalidade Federal</span><span class="pill">Origem ${lookupLoteriaName(f.loteria_id)}</span><span class="pill">Concurso ${f.concurso}</span><span class="pill">Data ${fmtDate(f.dt_sorteio)}</span><span class="pill">Fração ${fmtMoney(f.valor_fracao)}</span><span class="pill">Custo ${fmtMoney(f.valor_custo)}</span></div>`;
  });
  ...
  $('btn-limpar-fechamento').addEventListener('click', ()=>{ $('fec-qtd-vendida').value=''; $('fec-total').value=''; });
