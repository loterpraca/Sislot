...
if (tags) {
tags.innerHTML = ` <span class="rtag rtag-accent">
${fmtBRL(bolaoSel.valor_cota || 0)} </span>
...
console.error(
'Erro ao carregar lojas do encalhe:',
erro
);
    ...
    >
      ...
      <div class="encalhe-loja-card-valores">
        <div class="encalhe-loja-card-valor">
          <span>Saldo</span>
          <strong>
            ${encalheInteiro(loja.saldo_atual)}
          </strong>
        </div>
        ...
        <div class="encalhe-loja-card-valor">
          <span>Total</span>
          <strong>
            ${encalheInteiro(loja.total_disponivel)}
          </strong>
        </div>
      </div>
    </button>
...
`[${encalheInteiro(
        valores.encalhe_atual
      )}] → [${encalheInteiro(
        valores.novo_encalhe
      )}]`;
}
...
encalheInteiro(item.efeito_loja);
  ...
  return `
    <div class="encalhe-historico-item">
      ...
      ·
      ${encalheEscape(
        item.origem_nome || 'Origem'
      )}
      →
      ${encalheEscape(
        item.destino_nome || 'Destino'
      )}
...
box.innerHTML = lista
.map(item => ` <div class="encalhe-historico-item"> <strong>
[${encalheInteiro(item.qtd_anterior)}]
→
[${encalheInteiro(item.qtd_nova)}] </strong>
...
* A confirmação será ligada no próximo passo.
  */
  if (btnRevisar) {
